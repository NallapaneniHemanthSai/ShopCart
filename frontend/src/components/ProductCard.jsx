...
export default function ProductCard({ product, onAddToCart, onToggleWishlist, isWishlisted, isBestPrice }) {
  const outOfStock = product.stock <= 0;

  return (
    <div className="relative bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow flex flex-col overflow-hidden">
      {isBestPrice && (
        <span className="absolute top-3 right-3 z-10 text-[10px] font-bold uppercase tracking-wide text-white bg-emerald-600 px-2 py-1 rounded-full shadow-sm">
          Best Price
        </span>
      )}
      <Link to={`/products/${product.sku}`} className="p-4 flex-1 flex flex-col">
        ...
        <div className="mt-1">
          <StarRating rating={product.averageRating} count={product.reviewCount} size="text-xs" />
        </div>
        <p className="text-sm text-slate-500 mt-2 line-clamp-2 flex-1">{product.description}</p>
        <div className="mt-2 flex items-center gap-1 text-xs text-slate-500">
          <span>Sold by</span>
          <span className="font-medium text-slate-700">{product.vendorName}</span>
          {product.vendorVerified && (
            <span className="text-brand-600" title="Verified seller">✓</span>
          )}
        </div>
